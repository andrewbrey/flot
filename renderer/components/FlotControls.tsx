...
function FlotControls() {
	...
	React.useEffect(() => {
		if (urlInput.current)
			urlInput.current.value = url.replace(/^https?:\/\//, "").replace(/\/$/, "");
	}, [url]);
	...
	const handleDetachModeChange: (close: () => void) => ChangeEventHandler<HTMLInputElement> = (
		close
	) => {
		return (e) => {
			if (e.currentTarget.checked) {
										...
										<div className="relative flex flex-col items-start">
											...
											<div className="text-sm">
												<p id="video-focus-option-description" className="select-none">
													Add (experimental) styles to the flōted page which make videos as large as
													possible, hiding other content. Does not work on all websites, and resets
													to "off" when you change websites.
												</p>
											</div>
										</div>
										...
										<div className="relative flex flex-col items-start">
											...
											<div className="text-sm">
												<p id="detach-mode-option-description" className="select-none">
													Ignore <span className="font-bold">all</span> mouse clicks on the Flōt app
													window. Restore the ability to click by using your keyboard to re-activate
													the Flōt app window.
												</p>
											</div>
										</div>
