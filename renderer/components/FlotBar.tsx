    ...
    <React.Fragment>
      <nav
        className={cn(
          ...
          {
            'opacity-100': windowActive || childActive || windowHover || childHover || !url,
            'opacity-0': !(windowActive || childActive || windowHover || childHover || !url),
          }
        )}
      >
        <div
          className={cn('w-full h-full flex-grow flex items-center pointer-events-none select-none', {
            draggable: windowActive || childActive,
            'cursor-default': !(windowActive || childActive),
          })}
        >
          <p className="text-gray-300 text-lg font-light tracking-tighter">Flōt</p>
          <p className="text-transparent w-full h-full grid grid-cols-4 gap-2 overflow-hidden" aria-hidden>
        ...
        <div className="flex items-center justify-center">
          ...
          >
            <XIcon className="h-4 w-4 text-current inline-block" />
          </button>
        </div>
      </nav>
    </React.Fragment>
